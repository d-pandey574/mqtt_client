...
def main():
    ...
    try:
        ...
        print ("Fail to open file ", sys.argv[1])
        sys.exit(0)

    # Open the Serial Port
    try:
        ser = serial.Serial(port_list, 921600, timeout=30)
    except:
        print ("Please check to open the correct com port!!")
        sys.exit(0)

    # Ping serial port
    print ("Ping serial port.")
    payload_len = 1                    # Opcode (1)
    ser.write(struct.pack("B", payload_len))
    ser.write(PAYLOAD_JPG_OPCODE_PING)
    ...
    # # Send the filesize
    payload_len = 11                    # Opcode (1) + FileId(2) + FileSize(4) + CRC32(4)
    fileId = 1
    ser.write(struct.pack("B", payload_len))
    ser.write(PAYLOAD_JPG_OPCODE_FILEINFO_RSP)
    #Send the file Id
    ser.write(fileId.to_bytes(2, byteorder='little', signed=False))
    #Send the file size
